...
<body>
    <form action="edit.php?id=<?= $id ?>" method="POST">
        ...
        At least two words: <input type="text" name="two_words" value="<?= $two_words ?? $d["two_words"] ?>">
        <?= $errors["two_words"] ?? "" ?>
        <br>

        Exactly 7 chars: <input type="text" name="seven" value="<?= $seven ?? $d["seven"] ?>">
        <?= $errors["seven"] ?? "" ?>
        <br>
        ...
        <input type="checkbox" name="tickme" <?= ($tickme ?? $d["tickme"] == "on") == "on" ? "checked" : "" ?>> Must be checked
        <?= $errors["tickme"] ?? "" ?>
        <br>

        Favorite color: <select name="color">
            <option value="red" <?= ($color ?? $d["color"]) == "red" ? "selected" : "" ?>>Red</option>
            <option value="green" <?= ($color ?? $d["color"]) == "green" ? "selected" : "" ?>>Green</option>
            <option value="blue" <?= ($color ?? $d["color"]) == "blue" ? "selected" : "" ?>>Blue</option>
            <option value="other" <?= ($color ?? $d["color"]) == "other" ? "selected" : "" ?>>Other (not accepted)</option>
        </select>
        <?= $errors["color"] ?? "" ?>
        <br>
        
        Credit card number (XXXX-XXXX-XXXX-XXXX): <input type="text" name="card" value="<?= $card ?? $d["card"] ?>">
        <?= $errors["card"] ?? "" ?>
        <br>
